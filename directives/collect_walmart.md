# Directive: collect_walmart

**Status: BLOCKED — do not build yet.** Walmart has no licensed price API. Collection from its website or app waits on spec §11 Q1 (ToS and legal posture). Get a real answer, not an assumption.

## Contract, once unblocked
- Module `execution/collectors/walmart.py` exposes `collect(db, cfg) -> summary`
...
- Every raw response goes through `save_payload("walmart", body)`; its hash goes on each observation
...
- Stores go in `stores` with `store_id='walmart:<num>'`, county by coordinates
