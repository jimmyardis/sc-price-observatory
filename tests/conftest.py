...
from execution import config, export_regional, export_snapshot, fetch_bls, qa_checks
...
def tmp_dirs(tmp_path, monkeypatch):
    ...
    for mod in (config, _base, qa_checks, export_snapshot, export_regional, fetch_bls):
        monkeypatch.setattr(mod, "TMP", tmp, raising=False)
    monkeypatch.setattr(export_snapshot, "SNAPSHOTS", snaps)
    monkeypatch.setattr(export_regional, "SNAPSHOTS", snaps)
    return tmp, snaps
